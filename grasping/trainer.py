...
class MobileUNet(nn.Module):
    def __init__(self, pretrained: bool=True, num_rots: int=2) -> None:
        ...
        self.layers_narrow = torch.nn.Sequential(
            ...
            nn.ReLU(),
            nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)  # output is (4, 32, 32)
        )

        self.layers_wide = torch.nn.Sequential(
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True),  # output is (24, 32, 32)
            nn.Conv2d(24, 4, kernel_size=1, stride=1, padding=0)  # output is (4, 32, 32)
        )

        self.last_layers = torch.nn.Sequential(
            nn.Conv2d(4, 2, kernel_size=1, stride=1, padding=0), 
            nn.ReLU(), # output (2, 32, 32)
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)  # output is (2, 64, 64)
        )
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        x_combined = x_w + x_n # (B, 4, 32, 32)

        return self.last_layers(x_combined)  # (B, 2, 64, 64)
